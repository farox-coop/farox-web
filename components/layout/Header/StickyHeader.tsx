...
function StickyHeader({
  setShowStickyHeader,
  logoTextColor = "white",
  logoCharColor = "#6843E1",
  contentMaxWidthClassName = "max-w-screen-desktoplg",
}: StickyHeaderProps) {
  ...
  return (
    <div
      className="fixed top-0 left-0 w-full hidden bg-[#565656]/80
    laptop:block mx-auto z-50 px-8 scroll-smooth"
    >
      <header
        className={`bg-transparent py-5 grid grid-cols-[1fr_auto_1fr] items-center mx-auto ${contentMaxWidthClassName}`}
      >
        <div>
          <Link href={`/${locale}#home`} onClick={handleLogoClick}>
            <LogoFaroxSVG className="w-36" textColor={logoTextColor} charColor={logoCharColor} />
          </Link>
        </div>
        <nav>
