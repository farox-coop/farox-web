...
export async function GET(_request: Request, { params }: { params: Promise<{ locale: string }> }) {
  try {
    ...
    const contentDir = path.join(process.cwd(), "content", "not-so-light", locale)
    ...
    console.error("Error processing not-so-light projects:", error)
    return NextResponse.json({ error: "Error processing not-so-light projects" }, { status: 500 })
  }
}
