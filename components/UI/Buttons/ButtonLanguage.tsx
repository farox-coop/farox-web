...
function ButtonLanguage({ hoverItemsColor, language, languajeTextColor }: ButtonLanguageProps) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`lg:relative ${languajeTextColor} text-[12px] w-auto laptop:w-14 tablet:h-16 laptop:h-auto uppercase font-semibold laptop:text-base desktop:text-xl px-2 py-3 tablet:px-4 laptop:px-0 ${hoverItemsColor} ${
        locale === language
