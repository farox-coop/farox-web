...
function ServicesDropdown({ itemsColor, textActiveColor = "text-secondary", activeLineColor }: ServicesDropdownProps) {
  ...
  return (
    <li ref={containerRef} className="relative group" onMouseLeave={() => setIsOpen(false)}>
      <div className="flex items-center gap-1.5 px-2 py-1 transition-colors duration-200 group-hover:bg-black/54">
        <NavItem
          hoverItemsColor="group-hover:text-white"
        ...
        <button
          ...
          className={`${itemsColor} shrink-0 transition-all duration-150 group-hover:rotate-180 group-hover:text-white ${isOpen ? "rotate-180 text-white" : ""}`}
        >
      ...
      <div
        className={`pointer-events-none absolute left-0 top-full z-50 hidden w-115 pt-1 opacity-0 transition-all duration-150 ease-in-out group-hover:block group-hover:pointer-events-auto group-hover:opacity-100 ${isOpen ? "block! pointer-events-auto! opacity-100!" : ""}`}
      >
            ...
            <li key={item.key} className="transition-colors duration-150 hover:bg-white/10">
              ...
              >
                <span>
                  ...
                  <span className="mt-1 block text-sm font-light normal-case tracking-normal text-white/70">
                    {t(item.description)}
                  </span>
                </span>
